...
|---|---|---|---|
| ETF 후보 정보 | `etf_info`, ETF universe table | ETF 티커, 이름, 자산군, 상장일 확인 | 투자 가능 universe 정의 |
| 원천 가격 데이터 | 일별 ETF 가격 데이터 | ETF별 가격과 거래일 정보 | 기술지표 계산의 원자료 |
| 월말 가격 | `monthly_price` | 월말 기준 가격 정렬 | 월말 신호와 월간 수익률 연결 |
| 월간 수익률 | `monthly_return` | ETF별 월간 수익률 | 백테스트 성과 계산 |
| HSI 입력 신호 | `signal_inputs` | 수익률, 이동평균, 모멘텀, 변동성, 상대강도 | HSI 상태분류의 직접 입력 |
| 신호 방향 정의 | `signal_direction_map` | 각 지표가 좋은 신호인지 위험 신호인지 정의 | 부호 반전 여부 결정 |
| 점수화 설정 | `scaling_config`, `DEFAULT_PARAMS` | rolling z-score, clipping, 점수 범위 설정 | `s_j` 산출 |
| 상태분류 기준 | `theta`, `direction` 경계, accident 기준 | 신호 활성화 여부와 상태판정 기준 | HSI 5상태 분류 |
| 상태별 목표비중 | allocation rule table | HSI 상태를 ETF 목표비중으로 번역 | 포트폴리오 행동 규칙 |
...
따라서 “위험 악화 지표는 변동성뿐이다”라고 단순히 말하면 부족하다. 더 정확하게는, HSI는 모든 입력 신호를 방향 통일한 뒤 그 시점의 점수가 양수이면 위험 악화 성분 `V_plus`, 음수이면 위험 완화 성분 `V_minus`로 나누어 계산한다.
