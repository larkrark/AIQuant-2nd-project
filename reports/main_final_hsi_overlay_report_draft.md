...
프로젝트의 성격과 알파간의 정의를 명확히 하고자 한다. 알파의 정의를`RA 수익률 − BM 수익률`로 정의한다. 이 기준에서 Final_RA_dynamic_v1은 FixedBM_70_20_10 대비 OOS와 FULL 구간 모두에서 양의 수익률 알파를 달성했다고 보기 어렵다. OOS 기준 CAGR은 Final_RA_dynamic_v1이 18.65%, FixedBM_70_20_10이 20.69%였고, FULL 기준 CAGR도 Final_RA_dynamic_v1이 9.73%, FixedBM_70_20_10이 10.99%였다.
...
|---|---:|---:|---:|---:|---|
| OOS | 누적수익률 | 156.11% | 181.37% | -25.26%p | 수익률 알파 |
| OOS | CAGR | 18.65% | 20.69% | -2.04%p | 수익률 알파 |
...
| FULL | 누적수익률 | 275.72% | 341.62% | -65.90%p | 수익률 알파 |
| FULL | CAGR | 9.73% | 10.99% | -1.26%p | 수익률 알파 |
...
## 13.2 가상 포트폴리오 분석을 통한 BM 대비 성과 차이 해석

가상 포트폴리오 분석은 HSI 상태와 λ 실행속도 규칙에 따라 매월 계산된 ETF 비중이 실제 전략 성과에 어떤 영향을 주었는지 확인하는 과정이다. 본 프로젝트의 가상 포트폴리오는 실제 운용계좌가 아니라, 월말 HSI 상태와 λ 규칙으로 산출된 069500, 114260, 153130 비중을 다음 달 수익률에 적용한 백테스트 포트폴리오를 의미한다.

[표 13-3. 가상 포트폴리오 관점의 BM 대비 성과 차이 원인]

| 원인 항목 | 가상 포트폴리오에서 확인할 내용 | HSI 방법론과의 연결 | 성과 영향 |
|---|---|---|---|
| 위험자산 참여율 차이 | dynamic_v1의 069500 비중이 FixedBM의 70%보다 낮았던 구간 | HSI가 위험 경고 또는 충돌 상태를 감지하면 위험자산 목표비중을 낮춤 | 상승장에서는 누적수익률과 CAGR 제한 |
| 방어자산 비중 확대 | 114260, 153130 비중이 높아진 구간 | 위험상태에서 채권형·현금성 방어자산으로 이동 | MDD와 변동성 개선 |
| 회복장 복귀 지연 | 하락 이후 069500 비중이 빠르게 70% 수준으로 복귀하지 못한 구간 | λ 부분조정 구조가 목표비중 반영 속도를 늦춤 | 빠른 반등장에서 BM 대비 수익률 미달 가능 |
| 보수적 λ 조건 | 변동성·drawdown 조건에서 낮은 λ가 적용된 구간 | 신호 과잉 반응을 줄이는 대신 반응 속도 제한 | 회전율은 낮추지만 수익 상단 제한 가능 |
| 가격 기반 HSI 한계 | 외부 사건·수급·거래량·정책 요인을 충분히 반영하지 못한 부분 | 초기 구상 대비 실제 구현이 축소됨 | 시장 전이와 회복 시작점 포착 한계 |

Final_RA_dynamic_v1이 FixedBM_70_20_10보다 누적수익률과 CAGR이 낮았던 가장 직접적인 이유는 위험자산 참여율의 차이로 해석할 수 있다. FixedBM_70_20_10은 069500을 70%로 고정 보유하기 때문에 상승장에서는 구조적으로 높은 수익률을 얻기 쉽다. 반면 dynamic_v1은 HSI 상태와 λ 조건에 따라 위험 경고 또는 충돌 구간에서 069500 비중을 낮추고, 114260과 153130 비중을 높인다. 이 구조는 하락장에서는 MDD와 변동성을 줄이는 데 기여하지만, 상승장이나 빠른 반등장에서는 위험자산 참여율을 낮춰 BM 대비 수익률을 제한할 수 있다.

또한 λ 부분조정 구조는 목표비중으로 한 번에 이동하지 않고 일부만 이동하기 때문에, 급격한 비중 변화와 회전율을 줄이는 장점이 있다. 그러나 큰 하락 이후 시장이 빠르게 반등하는 구간에서는 위험자산 비중 확대가 지연될 수 있다. 따라서 dynamic_v1의 낮은 MDD와 변동성은 방어적 비중 조절의 결과이지만, 동시에 누적수익률과 CAGR이 FixedBM보다 낮아진 원인으로도 작용할 수 있다.

이 점은 HSI 방법론의 한계와도 연결된다. 초기 HSI 구상은 외부 경고 사건, 국내 본진 사건, 수급·거래량·매크로 전이, 큰 사건 이후 회복 시작점까지 해석하려는 넓은 구조였지만, 실제 과제 구현은 가격 기반 신호와 단순 MacroRisk 조건 중심으로 축소되었다. 따라서 HSI는 하락 위험을 완화하는 데 일정 부분 기여했지만, 회복장 재진입 시점과 위험자산 참여율을 최적으로 조절하는 데에는 한계가 있었다.

결과적으로 본 프로젝트의 HSI 기반 Overlay 전략은 BM을 수익률로 지속적으로 초과한 전략이라기보다, 위험자산 참여를 유지하면서도 낙폭과 변동성을 완화한 방어형 전략으로 해석하는 것이 적절하다. 후속 연구에서는 BM 대비 수익률 미달을 줄이기 위해 회복 신호 감지 기준, 위험자산 재진입 속도, 방향별 λ 조합, 외부 선행 사건과 국내 본진 사건의 분리 반영을 추가로 검토할 필요가 있다.
