...
이 절에서는 HSI 알고리즘과 λ 실행속도 조절이 어떤 방식으로 작동하는지 먼저 설명한다. 용어 정의와 변수명 설명은 본문 흐름을 가볍게 유지하기 위해 부록 A로 이동하였다. 본문에서는 전략이 실제로 어떻게 시장상태를 읽고, 목표비중을 만들고, 그 목표비중을 실제 포트폴리오에 반영하는지만 중심적으로 정리한다.


## 2.1 HSI의 역할: 예측모델이 아니라 상태 해석 지표

HSI는 특정 월의 수익률을 직접 맞히기 위한 예측모델이 아니다. 본 프로젝트에서 HSI는 여러 가격 기반 신호를 공통 기준 위에 놓고, 시장상태가 위험 완화 방향인지 위험 악화 방향인지 해석하기 위한 상태 해석 지표로 사용하였다. 따라서 HSI의 역할은 “다음 달 수익률이 얼마인가”를 예측하는 것이 아니라, “현재 포트폴리오가 위험자산 비중을 늘려도 되는 상태인가, 줄여야 하는 상태인가”를 판단하는 데 있다.

초기 구상에서는 수익률, 이동평균 대비 위치, 모멘텀, 변동성, 상대강도처럼 단위와 해석 방향이 서로 다른 신호를 그대로 더하지 않고, 한국시장 안에서 해석 가능한 기준으로 정렬한 뒤 분위수 또는 표준화 점수로 변환하려고 하였다. 이는 해외시장에서 통용되는 절대 기준이 한국시장에 그대로 맞지 않을 수 있기 때문이다. 예를 들어 어떤 기술지표가 해외시장에서는 큰 음수 구간에서 위험 신호로 작동하더라도, 한국시장에서는 더 좁은 범위의 음수 구간이 위험 신호로 해석될 수 있다. 이 경우 동일한 절대 기준을 적용하기보다, 한국시장 자료에서 관찰되는 위험 신호 구간을 기준으로 중심점을 다시 설정하고 여러 신호를 비교 가능한 형태로 만드는 것이 필요하다.
...
* **방향성(Direction):** 위험 악화 신호($V_{plus}$)와 위험 완화 신호($V_{minus}$)의 차이를 구합니다.
  * 계산식: $Direction = V_{plus} - V_{minus}$
  * 이 값이 양수이면 시장이 위험한 쪽으로 기울고 있음을, 음수이면 안전한 쪽으로 기울고 있음을 의미합니다.
* **절대 강도(Intensity):** 신호의 방향과 관계없이, 지금 시장에서 발생하는 움직임이 얼마나 강력한지를 나타냅니다.
  * 계산식: $Intensity = |V_{plus}| + |V_{minus}|$
* **충돌도(Conflict):** 위험 신호와 안전 신호가 동시에 강하게 발생할 때 시장은 혼란을 겪습니다.
  * 계산식: $Conflict = V_{plus} \times V_{minus}$
  * 이 세 가지 지표를 통해 시장이 단순한 상승/하락을 넘어, 얼마나 혼란스러운 상태인지 입체적으로 파악합니다.



본 프로젝트의 구현에서는 이 초기 구상을 과제 범위에 맞게 단순화하였다. 가격 기반 신호를 이용해 HSI 상태를 만들고, HSI 상태를 ETF 목표비중으로 연결하였다. 따라서 HSI는 완성된 예측 엔진이라기보다, 한국 ETF 방어형 RoboAdvisor 전략에서 위험상태를 해석하고 목표비중을 정하기 위한 실험적 overlay 지표로 해석하는 것이 적절하다.

## 2.2 HSI 상태가 목표비중으로 연결되는 방식

HSI가 시장 상태를 해석하면, 이 정보를 ETF 비중 조절 규칙으로 번역한다. 이때 중요한 것은 즉각적인 반응이 아닌 점진적인 대응이다. HSI는 그 자체로 매매 명령을 내리는 예측 모델이 아니라, 이미 정해진 유니버스 내에서 위험자산 비중을 결정하는 보조 장치(Overlay)로 작동하게 된다.
...
## 2.4 MDD와 위험조정 성과가 개선될 수 있는 작동 원리
...
## 2.5 성과 해석상 주의: 초과수익률 알파와 방어형 성과의 구분
...
![Monte Carlo 100회 단위 진행 확인](../output/figures/main_final_fig_38b_monte_carlo_progress_by_100.png)

[그림 13-3. Monte Carlo 100회 단위 진행 확인]

그림 13-3은 placebo simulation이 100회 단위로 누적되어 최종 1,000회까지 수행되었음을 보여준다. 이 그림은 최종 결과가 단일 셔플에 의존한 것이 아니라, 반복 표본을 단계적으로 누적해 만든 Monte Carlo 검정 결과임을 시각적으로 확인하기 위한 자료이다.
