...
## 6. 검증 흐름 안내백서: 본 프로젝트의 판단 기준

본 프로젝트는 단순히 백테스트 수익률이 높은 조합을 찾는 방식으로 진행하지 않았다. HSI overlay 전략은 규칙 기반 ETF 자산배분 구조이므로, 딥러닝 모델의 EarlyStopping을 그대로 적용하기보다 금융 백테스트에 맞는 검증 기준을 세웠다.

우리는 다음 세 가지 기준을 기본으로 삼았다.

### 6.1 백테스트 검증을 기본으로 했다

백테스트 단계에서는 신호 시점, 비중 적용, 비교군, 비용, 손실구간이 정합적인지 확인하는 것을 기본 원칙으로 삼았다.

```text
월말 HSI 신호
→ 다음 달 ETF 수익률에 적용
→ 동일 기간·동일 ETF 유니버스에서 BM과 비교
→ CAGR뿐 아니라 MDD, Sharpe, Sortino, Calmar, Turnover를 함께 확인
```

따라서 본 프로젝트는 같은 달 수익률을 미리 보고 비중을 정하는 구조를 피하고, 월말에 관측 가능한 신호를 다음 달 수익률에 적용하는 방식으로 look-ahead bias 가능성을 줄였다.

또한 HSI 후보를 단독으로 평가하지 않고, Fixed 70/20/10 BM, EW Benchmark, HSI baseline, Lambda 후보를 같은 ETF 유니버스와 같은 기간에서 비교하였다.

### 6.2 최고 CAGR이 아니라 후보 선별 기준을 기본으로 했다

본 프로젝트는 최고 CAGR 하나를 찾는 방식으로 후보를 선택하지 않았다. 후보 선별에서는 다음 기준을 함께 보았다.
...
따라서 HSI baseline은 CAGR 측면에서 의미가 있었더라도, MDD와 Turnover 부담이 커 최종 후보로 채택하지 않았다. 반대로 Lambda 0.1과 Lambda 0.3은 baseline 대비 비중 전환을 완화하면서도, 각각 저회전·보수형 후보와 수익성·Calmar 균형형 후보로 해석 가능했기 때문에 최종 우선 후보로 남겼다.

이 기준은 실험 범위를 계속 넓혀 좋은 결과가 나올 때까지 찾는 방식과 다르다. 추가 layer를 붙였더라도 성과 개선 폭이 작거나 Turnover, Calmar, 비용 측면의 대가가 크면 최종 후보로 올리지 않았다.

### 6.3 Robustness 검증을 기본으로 했다

Robustness 검증은 결과가 항상 좋다는 것을 증명하는 단계가 아니라, 어떤 조건에서 유지되고 어떤 조건에서 약해지는지 투명하게 확인하는 단계로 보았다.

본 프로젝트에서는 다음 항목을 통해 최종 후보를 흔들어 보았다.

```text
기간분할 robustness
HSI 상태별 조건부 진단
069500 하위 10% 큰 손실월 진단
거래비용 민감도
macro overlay trade-off
Fixed 70/20/10 BM을 포함한 benchmark alignment
```

그 결과 Lambda 0.3은 전체 기간 CAGR과 Calmar 측면에서 균형형 후보로 남았고, Lambda 0.1은 낮은 Turnover와 큰 손실월 방어 측면에서 보수형 후보로 남았다. 반면 macro companion은 MDD를 아주 소폭 낮추는 방향으로 작동했지만, CAGR·Calmar·Turnover 측면의 비용이 함께 발생하여 최종 후보를 대체하지 못했다.

### 6.4 본 프로젝트의 검증 흐름

본 프로젝트의 검증 흐름은 다음과 같이 정리한다.

```text
1단계: 백테스트 검증
- 신호 시점과 수익률 적용 시점 분리
- 동일 기간·동일 ETF 유니버스 기준 비교
- 성과지표와 Drawdown, Turnover 산출

2단계: 후보 선별
- 최고 CAGR이 아니라 MDD, Calmar, Turnover, 비용, 해석 가능성 종합 판단
- HSI baseline은 내부 기준선으로 남김
- Lambda 0.1과 Lambda 0.3을 운용 목적별 후보로 분리

3단계: Robustness 검증
- 기간분할, HSI 상태별, 큰 손실월, BM alignment로 최종 후보를 재점검
- macro companion은 최종 후보가 아니라 보조 진단 layer로 정리
```

따라서 본 프로젝트는 “최적 전략을 찾았다”기보다, 금융 백테스트 기준에 따라 검증 가능한 RoboAdvisor 후보를 선별한 프로젝트로 해석한다.

보고서 표현은 다음처럼 정리한다.

```text
본 프로젝트는 최고 수익률 하나를 찾는 것이 아니라,
동일 ETF 유니버스 안에서 benchmark, 내부 기준선, Lambda 후보를 비교하고,
MDD, Calmar, Turnover, 거래비용 민감도, 기간분할 robustness를 함께 고려하여
해석 가능한 RoboAdvisor 후보를 선별하였다.
```
