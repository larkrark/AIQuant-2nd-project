...
class ExperimentConfig:
    ...
    hsi_weight_file_candidates: Tuple[str, ...] = (
        # 실제 프로젝트 산출물: apply_date 기준으로 이미 해당 월 수익률에 적용되는 비중
...
def read_hsi_weights_if_available(root: Path, ret_index: pd.DatetimeIndex) -> Tuple[pd.DataFrame | None, str]:
    """
    HSI dynamic_v1 weight 파일을 읽는다.

    지원 형식 A: apply_date, signal_date, w_069500, w_114260, w_153130
        - 이 파일은 이미 apply_date 수익률에 적용되는 applied weights이다.
        - 따라서 backtest에서 추가 shift를 하면 안 된다.

    지원 형식 B: date/month, 069500, 114260, 153130
        - 이 파일은 signal weights로 간주한다.
        - 기존 backtest 엔진에서 shift(1)를 적용한다.
    """
    for rel in CFG.hsi_weight_file_candidates:
        ...
        try:
            ...
            # 형식 A: main_final_portfolio_composition_dynamic_v1.csv
            if "apply_date" in cols and all(c in cols for c in ["w_069500", "w_114260", "w_153130"]):
                df = raw.copy()
                df["apply_date"] = pd.to_datetime(df["apply_date"])
                df = df.set_index("apply_date").sort_index()
                w = df[["w_069500", "w_114260", "w_153130"]].rename(
                    columns={
                        "w_069500": CFG.equity,
                        "w_114260": CFG.bond,
                        "w_153130": CFG.cash_like,
                    }
                )
                w = w.apply(pd.to_numeric, errors="coerce")
                w = w.reindex(ret_index).ffill()
                print(f"[OK] HSI dynamic_v1 applied weights loaded: {p}")
                print("     detected format: apply_date + w_ columns, so no additional shift will be applied")
                return normalize_weights(w), "applied"
            ...
            if all(c in df.columns for c in needed):
                ...
                w = w.reindex(ret_index).ffill()
                print(f"[OK] HSI dynamic_v1 signal weights loaded: {p}")
                print("     detected format: ticker columns, so shift(1) will be applied")
                return normalize_weights(w), "signal"

        except Exception as e:
            print(f"[WARN] HSI weight 후보 읽기 실패: {p} / {e}")

    print("[WARN] HSI dynamic_v1 weight 파일을 자동으로 찾지 못했습니다.")
    print("       이번 실행에서는 HSI_dynamic_v1 없이 canary proxy와 BM만 계산합니다.")
    return None, "missing"
...
def backtest_from_applied_weights(
    ret: pd.DataFrame,
    ...
    """
    이미 apply_date 수익률에 적용되는 비중을 사용해 백테스트한다.

    main_final_portfolio_composition_dynamic_v1.csv는 apply_date, signal_date를 함께 갖고 있으며,
    w_069500/w_114260/w_153130은 apply_date 월 수익률에 적용되는 실제비중이다.
    따라서 여기서는 shift(1)를 추가로 적용하지 않는다.
    """
    w = applied_weights.reindex(ret.index).ffill()
...
def main() -> None:
    ...
    hsi_w, hsi_weight_mode = read_hsi_weights_if_available(root, ret.index)
    ...
    for name, w in weights.items():
        if name == "HSI_dynamic_v1" and hsi_weight_mode == "applied":
            r, tv = backtest_from_applied_weights(ret, w, CFG.cost_bp, name)
        else:
