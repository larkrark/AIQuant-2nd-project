...
def check_missing_values(prices, universe=None):
    ...
    # 연도별 결측치 수: groupby.apply/drop(columns='year')를 쓰지 않는 안전한 방식
    yearly_missing = prices.isna().groupby(prices.index.year).sum().T
...
# ── 입력 #13) 비중 규칙 후보 — 그리드서치 파트 연결 예정 ──
WEIGHT_RULE_CONFIG = None        # weight_rule_config.csv (HSI 설계 파트)

# ── 입력 #14) 거래비용 가정 — 민감도 분석 파트 연결 예정 ──
COST_ASSUMPTION_CONFIG = None    # cost_assumption_config.csv (백테스트 파트)


# 14개 입력 항목 명세 (docx 'HSI 입력 구조표' 1:1 대응)
...
HSI_INPUT_STRUCTURE = [
    ...
    {"no": 13, "key": "weight_rule_config",
     "category": "비중 규칙 후보", "object": "weight_rule_config.csv",
     "columns": "strategy_name, conflict_weight, risk_warning_weight, "
                "accident_zone_weight, note",
     "description": "HSI 상태별 위험자산 비중 후보(Grid Search 입력 설정).",
     "next_step": "비중 Grid Search",
     "source": "WEIGHT_RULE_CONFIG (placeholder)", "status": "예정"},
    {"no": 14, "key": "cost_assumption_config",
     "category": "거래비용 가정", "object": "cost_assumption_config.csv",
     "columns": "cost_rate, label, note",
     "description": "Turnover에 곱해 비용 반영 성과를 계산하는 비용률 후보.",
     "next_step": "거래비용 민감도 분석",
     "source": "COST_ASSUMPTION_CONFIG (placeholder)", "status": "예정"},
]
...
def print_hsi_input_table(df=None):
    ...
    _print_grid(headers, rows, aligns)
    print()
    print("  ※ 필수 컬럼·설명·다음 단계는 hsi_input_structure.csv 참고")
    print()
...
def collect_hsi_inputs(prices=None, volumes=None, universe=None, params=None):
    ...
    inputs["weight_rule_config"]     = WEIGHT_RULE_CONFIG
    inputs["cost_assumption_config"] = COST_ASSUMPTION_CONFIG
    return inputs
...
# owner: "이 모듈" = 본 파일(주원 파트)이 산출 | "HSI 설계" | "백테스트"
# status: "산출"(현재 생성 가능) | "부분" | "예정"
HSI_OUTPUT_STRUCTURE = [
    {"no": 1,  "category": "ETF 유니버스 표", "object": "selected_etf_universe.csv",
     "columns": "ticker, name, asset_class, risk_group, usable_flag, note",
     "meaning": "최종적으로 실험에 사용할 ETF 목록.",
     "target": "팀 전체 / 백테스트",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_etf_info_table() / select_etf()"},
    {"no": 2,  "category": "결측치 점검표", "object": "missing_value_check.csv",
     "columns": "ticker, missing_count, missing_ratio, comment",
     "meaning": "ETF별 결측치 현황 확인.",
     "target": "데이터 품질 검토",
     "owner": "이 모듈", "status": "산출",
     "producer": "check_missing_values()"},
    {"no": 3,  "category": "사용 가능 기간 점검표", "object": "available_period_check.csv",
     "columns": "ticker, start_date, end_date, usable_years",
     "meaning": "ETF별 실제 사용 가능 기간 확인.",
     "target": "유니버스 확정",
     "owner": "이 모듈", "status": "산출",
     "producer": "check_data_period()"},
    {"no": 4,  "category": "월말 가격표", "object": "monthly_prices.csv",
     "columns": "Date, ETF별 월말 가격 컬럼",
     "meaning": "리밸런싱 기준 가격표.",
     "target": "월간 수익률 및 백테스트",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_monthly_price_table()"},
    {"no": 5,  "category": "월간 수익률표", "object": "monthly_returns.csv",
     "columns": "Date, ETF별 월간 수익률 컬럼",
     "meaning": "전략 성과 계산의 기본 입력.",
     "target": "백테스트",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_monthly_return_table()"},
    {"no": 6,  "category": "HSI 입력 신호표", "object": "hsi_signal_inputs.csv",
     "columns": "ret_1m, ret_3m, ma_gap, momentum, volatility, relative_strength "
                "(+확장: ret_6m, ret_12m, drawdown, shock_count, defensive_rs)",
     "meaning": "HSI 계산 직전 단계의 원신호 데이터(부호 반전 전 실제값).",
     "target": "HSI 계산",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_hsi_signal_inputs()"},
    {"no": 7,  "category": "HSI 방향 점수표", "object": "hsi_direction_scores.csv",
     "columns": "Date, ETF, direction_score",
     "meaning": "위험 악화/완화 방향성 점수.",
     "target": "상태 분류",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_hsi_signal_table()['hsi_direction']"},
    {"no": 8,  "category": "HSI 강도 점수표", "object": "hsi_intensity_scores.csv",
     "columns": "Date, ETF, intensity_score",
     "meaning": "신호가 얼마나 강한지 나타내는 점수.",
     "target": "상태 분류",
     "owner": "이 모듈", "status": "산출",
     "producer": "compute_hsi() — *_intensity 컬럼"},
    {"no": 9,  "category": "HSI 상태 분류표", "object": "hsi_state_table.csv",
     "columns": "Date, ETF, state_label",
     "meaning": "안정/중립/충돌/위험/사고 등 HSI 상태 이름.",
     "target": "비중 조절 규칙 적용",
     "owner": "이 모듈", "status": "부분",
     "producer": "make_hsi_signal_table()['hsi_signal'] (3단계)"},
    {"no": 10, "category": "HSI 종합 요약표", "object": "hsi_summary.csv",
     "columns": "Date, direction, intensity, conflict, state",
     "meaning": "HSI 핵심 요약 결과.",
     "target": "발표 / 보고서",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_hsi_signal_table()['snapshot']"},
    {"no": 11, "category": "비중 조절 규칙표", "object": "allocation_rule_table.csv",
     "columns": "state, risk_asset_action, defensive_asset_action",
     "meaning": "상태별 행동 규칙 정리.",
     "target": "리밸런싱 실행",
     "owner": "HSI 설계", "status": "예정",
     "producer": "HSI_gridsearch_experiment.py"},
    {"no": 12, "category": "리밸런싱 비중표", "object": "rebalance_weights.csv",
     "columns": "Date, 자산별 비중",
     "meaning": "실제 전략이 적용된 비중 결과.",
     "target": "백테스트",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 13, "category": "백테스트 시계열", "object": "backtest_timeseries.csv",
     "columns": "Date, portfolio_return, cum_return, drawdown",
     "meaning": "전략의 기간별 성과 흐름.",
     "target": "성과평가",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 14, "category": "성과 요약표", "object": "performance_summary.csv",
     "columns": "CAGR, MDD, Sharpe, Volatility, Calmar, Sortino, "
                "final_cum_return, monthly_win_rate, DefenseScore",
     "meaning": "전략별 핵심 성과 비교표(MDD·Calmar 중점).",
     "target": "비교 전략 검증",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 15, "category": "그리드서치 결과표", "object": "grid_search_results.csv",
     "columns": "method, strategy_name, conflict_weight, risk_warning_weight, "
                "accident_zone_weight, CAGR, MDD, Sharpe, Calmar, "
                "avg_turnover, max_turnover, rank",
     "meaning": "다양한 비중 규칙 조합의 결과 비교.",
     "target": "최적 후보 검토",
     "owner": "HSI 설계", "status": "예정",
     "producer": "HSI_gridsearch_experiment.py"},
    {"no": 16, "category": "Robustness 결과표", "object": "robustness_results.csv",
     "columns": "실험 조건, 성과지표, 안정성 평가",
     "meaning": "기간·위기구간·비용률 변화에도 결과 유지 여부 확인.",
     "target": "최종 해석",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 17, "category": "전처리 로그", "object": "preprocessing_log.md",
     "columns": "데이터 처리 과정, 제외 사유, 수정 내역",
     "meaning": "재현성과 설명 가능성 확보(처리 단계·제외 사유 기록).",
     "target": "보고서 / 회의 공유",
     "owner": "이 모듈", "status": "산출",
     "producer": "PreprocessingLog / write_preprocessing_log()"},
    {"no": 18, "category": "신호 방향 정의표", "object": "signal_direction_summary.csv",
     "columns": "signal_name, direction_rule, hsi_interpretation",
     "meaning": "각 신호의 위험 악화/완화 방향 정리(부호 반전 근거).",
     "target": "HSI 계산 설명 / 보고서",
     "owner": "이 모듈", "status": "산출",
     "producer": "make_signal_direction_table()"},
    {"no": 19, "category": "점수화 방식 비교표", "object": "scaling_method_comparison.csv",
     "columns": "Date, ETF, signal, raw_value, rank_score, zscore_score",
     "meaning": "rank와 z-score 해석 차이 비교.",
     "target": "시각화 / 방법론 설명",
     "owner": "HSI 설계", "status": "예정",
     "producer": "HSI_gridsearch_experiment.py"},
    {"no": 20, "category": "월말 HSI 상태표", "object": "hsi_monthly_state.csv",
     "columns": "signal_date, ETF, state, direction, intensity, conflict",
     "meaning": "일별 HSI 중 월말 상태만 추출.",
     "target": "다음 달 수익률 정렬",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 21, "category": "HSI-수익률 정렬표", "object": "hsi_return_alignment_check.csv",
     "columns": "signal_date, next_return_date, state, next_month_return",
     "meaning": "월말 HSI 상태가 다음 달 수익률에 적용됐는지 확인(look-ahead bias 점검).",
     "target": "백테스트 검증",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 22, "category": "Turnover 요약표", "object": "turnover_summary.csv",
     "columns": "strategy, method, avg_turnover, max_turnover, total_turnover",
     "meaning": "전략별 매매 빈도·비중 변화 크기 확인.",
     "target": "비용 분석 / 후보 필터링",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 23, "category": "거래비용 민감도 결과표", "object": "transaction_cost_sensitivity.csv",
     "columns": "strategy, method, cost_rate, CAGR_net, MDD_net, Sharpe_net, Calmar_net",
     "meaning": "거래비용률 변화 후 성과 유지 여부 확인.",
     "target": "Robustness / 최종 후보 판단",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
    {"no": 24, "category": "후보 필터링 결과표", "object": "candidate_filtering_result.csv",
     "columns": "strategy, method, mdd_pass, turnover_pass, cost_pass, "
                "robustness_pass, candidate_status",
     "meaning": "Grid Search 후보의 탈락/보류/후보 정리.",
     "target": "최종 후보 선정",
     "owner": "백테스트", "status": "예정",
     "producer": "백테스트 파트"},
]
...
def make_hsi_output_table():
    """HSI 출력 구조표(24개 항목)를 산출물 카탈로그 DataFrame으로 반환."""
    rows = []
...
def print_hsi_output_table(df=None):
    """HSI 출력 구조표(24개 산출물)를 콘솔 격자 표로 출력 (한글 폭 보정)."""
    if df is None:
        df = make_hsi_output_table()

    status_mark = {"산출": "✓ 산출", "부분": "△ 부분", "예정": "· 예정"}
    ...
    print("=" * 100)
    print("[추가 기능 2] HSI 출력 구조표 — 24개 산출물 카탈로그")
    print("=" * 100)
    print(f"  총 {n_total}개 산출물  |  현재 산출 가능: {n_now}개  |  "
          f"부분 산출: {n_part}개  |  예정: {n_total - n_now - n_part}개")
    print()

    headers = ["번호", "출력 구분", "파일명/객체", "담당", "상태", "생성 함수"]
    aligns  = ["right", "left", "left", "center", "center", "left"]
    rows = []
    for _, r in df.iterrows():
        rows.append([
            str(r["번호"]), r["출력 구분"], r["파일명/객체"],
            r["담당"], status_mark.get(r["상태"], r["상태"]), r["생성 함수"],
        ])
    _print_grid(headers, rows, aligns)
    print()
    print("  범례  ✓ 산출=본 모듈에서 바로 생성  |  △ 부분=일부만/수기 보강  |  · 예정=타 파트")
    print("  ※ 주요 컬럼·의미·전달 대상 전체 내용은 hsi_output_structure.csv 참고")
    print()
...
if __name__ == "__main__":
    ...
    # ── [추가 기능 1] HSI 입력 구조표 (14개 항목) ──────────────
    input_df = make_hsi_input_table()
    print_hsi_input_table(input_df)
    input_df.to_csv("hsi_input_structure.csv", index=False, encoding="utf-8-sig")

    # 신호 방향 정의표(입력 #11 / 출력 #18) — 데이터 없이 산출 가능
    sig_dir_df = make_signal_direction_table()
    sig_dir_df.to_csv("signal_direction_map.csv", index=False, encoding="utf-8-sig")
    print("→ hsi_input_structure.csv / signal_direction_map.csv 저장 완료\n")

    # ── [추가 기능 2] HSI 출력 구조표 (24개 산출물 카탈로그) ────
    output_df = make_hsi_output_table()
    print_hsi_output_table(output_df)
    output_df.to_csv("hsi_output_structure.csv", index=False, encoding="utf-8-sig")
    print("→ hsi_output_structure.csv 저장 완료\n")
    ...
    try:
        ...
        try:
            ...
            print(f"[주의] 데이터 처리 중단: {e}")
            print("가격 데이터 로드 이후 결측치/유동성/HSI 산출 단계에서 중단되었을 수 있습니다.")
            log.step(f"중단: 데이터 처리 실패 — {e}", echo=False)
            log.save("preprocessing_log.md")
            print("→ preprocessing_log.md 저장 완료 (중단 지점까지 기록)")
        ...
        # ── ⑥ [추가 기능 1·2] HSI 기본 입력 신호표 (원신호) ───
        signal_inputs = make_hsi_signal_inputs(
            prices, include_extended=True, log=log)
        print_hsi_signal_inputs(signal_inputs)
        signal_inputs.to_csv(
            "hsi_signal_inputs.csv", index=False, encoding="utf-8-sig")
        print("→ hsi_signal_inputs.csv 저장 완료\n")
        ...
        # ── [추가 기능 1] 14개 입력 항목을 실제 객체로 수집 ─────
        hsi_inputs = collect_hsi_inputs(prices=prices, volumes=volumes)
        print_collected_inputs(hsi_inputs)

        # ── [추가 기능 3] 전처리 로그 저장 ───────────────────
        log.note("산출물: 선정 보고서, 자산군 분류표, 입출력 구조표, "
                 "월말 가격/수익률표, HSI 입력 신호표, HSI 점수표")
        log.save("preprocessing_log.md")
        ...
        # 실패해도 여기까지의 처리 과정과 실패 사유를 로그로 남김
