...
산출물 체크리스트 (주원 파트):
  ...
  ⑦ 월간 수익률표                → make_monthly_return_table_pct() / make_monthly_return_table_decimal()
...
# ──────────────────────────────────────────────────────────────
# 0. 프로젝트 경로 설정
#    실행 위치와 무관하게 프로젝트 루트 기준으로 산출물을 저장한다.
# ──────────────────────────────────────────────────────────────
...
WORKBOOK_PATH = OUTPUT_TABLE_DIR / "hsi_data_bundle.xlsx"
PREPROCESSING_LOG_PATH = DOCS_DIR / "preprocessing_log.md"
...
def make_monthly_return_table_pct(prices):
    """
    ⑦-A 월간 수익률표 생성 — pct 단위.
...
def make_monthly_return_table_decimal(prices):
    """
    ⑦-B 월간 수익률표 생성 — decimal 단위.

    백테스트 연결용 월간 수익률을 decimal 단위로 반환.
    예: 2.5% → 0.025
    인덱스: 연월(YYYY-MM), 컬럼: ticker

    Returns
    -------
    pd.DataFrame — 월간 수익률 테이블 (단위: decimal)
    """
    monthly_ret = make_monthly_price_table(prices).pct_change()
    return monthly_ret.round(6).dropna(how="all")
...
def make_monthly_return_table(prices):
    ...
    기본 반환값은 pct 단위이다.
    백테스트에는 make_monthly_return_table_decimal()을 사용한다.
    """
    return make_monthly_return_table_pct(prices)
...
def make_hsi_input_table():
    """HSI 입력 구조표(데이터 파트 12개 항목)를 DataFrame으로 반환."""
    rows = []
...
def print_hsi_input_table(df=None):
    ...
    print("=" * 96)
    print("[추가 기능 1] HSI 입력 구조표 — 데이터 파트 12개 입력 항목")
    print("=" * 96)
...
def collect_hsi_inputs(prices=None, volumes=None, universe=None, params=None):
    """
    데이터 파트 입력 항목을 실제 객체로 '받아와서' 하나의 dict로 묶어 반환.
...
HSI_OUTPUT_STRUCTURE = [
    ...
    {"no": 4, "role": "②가격·수익률", "category": "월간 수익률표(pct/decimal)",
     "columns": "year_month, ticker, monthly_return_pct / monthly_return_decimal",
     "meaning": "pct 단위는 검수·보고용, decimal 단위는 백테스트 연결용으로 사용한다.",
     "producer": "make_monthly_return_table_pct() / make_monthly_return_table_decimal()", "bundle_table": "monthly_return_pct / monthly_return_decimal", "status": "산출"},
    # ── ③ HSI 입력 신호 ────────────────────────────────────────
    ...
    # ── ④ 점수화 전후 중간 산출물 ──────────────────────────────
    {"no": 6, "role": "④점수화 중간", "category": "지표 점수표(hsi_scaled_scores)",
     "columns": "date, ticker, score_return/ma_pos/momentum/vol/rs (-10~+10)",
     "meaning": "원자료가 아니라 표준화·부호 통일·스케일링이 끝난 지표별 HSI 점수.",
     "producer": "make_hsi_signal_table()['raw_scores']", "bundle_table": "hsi_scaled_scores", "status": "산출"},
...
def make_workbook_meta(source_file=None):
    """
    hsi_data_bundle.xlsx의 안내용 meta 시트를 생성한다.

    다른 조원이 워크북을 열었을 때 데이터 시작일, 벤치마크,
    수익률 단위, ETF 유니버스, 파일 역할을 바로 확인할 수 있도록 한다.
    """
    selected_etfs = ", ".join(ETF_UNIVERSE.keys())
    source_file = source_file or Path(__file__).name
    rows = [
        {"field": "project_name", "value": "HSI dynamic allocation data bundle"},
        {"field": "generated_at", "value": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")},
        {"field": "source_file", "value": str(source_file)},
        {"field": "project_root", "value": str(PROJECT_ROOT)},
        {"field": "data_start_date", "value": DATA_START_DATE},
        {"field": "benchmark_ticker", "value": BENCHMARK_TICKER},
        {"field": "selected_etfs", "value": selected_etfs},
        {"field": "monthly_return_pct", "value": "검수·보고용. 예: 2.5% → 2.5"},
        {"field": "monthly_return_decimal", "value": "백테스트 연결용. 예: 2.5% → 0.025"},
        {"field": "signal_inputs", "value": "부호 반전 전 실제 원신호"},
        {"field": "hsi_scaled_scores", "value": "원자료가 아니라 표준화·부호 통일·스케일링이 끝난 HSI 점수"},
        {"field": "market_event_calendar", "value": "전략 입력이 아니라 사후 해석·시각화·위기구간 검증용"},
    ]
    return pd.DataFrame(rows)


def build_data_workbook(*, meta=None, etf_info=None, asset_class=None,
                        monthly_price=None, monthly_return=None,
                        monthly_return_pct=None, monthly_return_decimal=None,
                        signal_inputs=None, raw_scores=None, hsi_scaled_scores=None,
    ...
    """
    # 기존 호출 호환: monthly_return 인자는 pct 단위로 간주한다.
    if monthly_return_pct is None and monthly_return is not None:
        monthly_return_pct = monthly_return

    # 기존 호출 호환: make_hsi_signal_table()['raw_scores']는 실제로 scaled score이다.
    if hsi_scaled_scores is None and raw_scores is not None:
        hsi_scaled_scores = raw_scores
    ...
    spec = [
        ...
        ("monthly_return_pct",    monthly_return_pct,     True),    # index=연월, 2.5% → 2.5
        ("monthly_return_decimal", monthly_return_decimal, True),   # index=연월, 2.5% → 0.025
        ("signal_inputs",        signal_inputs,          False),   # Date 컬럼 보유
        ("hsi_scaled_scores",    hsi_scaled_scores,      True),    # index=날짜, -10~+10 점수
...
def save_data_workbook(sheets, path=WORKBOOK_PATH):
    """
    ...
    try:
        ...
        raise ImportError(
            "hsi_data_bundle.xlsx 저장을 위해 openpyxl이 필요합니다. "
            "다음 명령어를 실행하세요: python -m pip install openpyxl"
        ) from e
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        for name, (df, keep_index) in sheets.items():
            df.to_excel(writer, sheet_name=str(name)[:31], index=keep_index)
    return path
...
if __name__ == "__main__":
    ...
    meta_df = make_workbook_meta()                    # 워크북 안내용 meta 시트
    ...
    monthly_price_df = monthly_return_pct_df = monthly_return_decimal_df = None
    ...
    try:
        ...
        monthly_return_pct_df = make_monthly_return_table_pct(prices)
        monthly_return_decimal_df = make_monthly_return_table_decimal(prices)
        log.step("월말 가격표·월간 수익률표(pct/decimal) 리샘플링 완료")
    ...
    sheets = build_data_workbook(
        ...
        monthly_return_pct=monthly_return_pct_df,
        monthly_return_decimal=monthly_return_decimal_df,
        signal_inputs=signal_inputs,
        hsi_scaled_scores=(signal_tables or {}).get("raw_scores"),
    ...
    saved_workbook_path = save_data_workbook(sheets, WORKBOOK_PATH)
    print_data_workbook_summary(sheets, saved_workbook_path)
    log.step(f"산출물 합본 저장: {saved_workbook_path} ({len(sheets)}개 시트)", echo=False)

    # ── 전처리 로그 저장 (사람용 보고서) ──────────────────────
    log.note("산출물은 hsi_data_bundle.xlsx 하나로 합본 (표별 시트 분리)")
    log.note("Grid Search / Robustness / Turnover·거래비용 / 백테스트는 후속 파트 담당(본 모듈 제외)")
    PREPROCESSING_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    saved_log_path = log.save(PREPROCESSING_LOG_PATH)
    print(f"→ {saved_log_path} 저장 완료\n")

    print("=" * 80)
    print(f"데이터 파트 산출물 생성 완료 → {WORKBOOK_PATH} + {PREPROCESSING_LOG_PATH}")
