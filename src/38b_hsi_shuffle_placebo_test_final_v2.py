...
output/figures/main_final_fig_38b_monte_carlo_progress_by_100.png
output/figures/main_final_fig_38b_oos_percentile_stability_by_100.png
...
def configure_matplotlib_font() -> None:
    """Windows/Korean environment에서 한글 그림 제목이 깨지지 않도록 폰트를 설정한다."""
...
OUTPUT_PROGRESS_CHECKPOINTS = TABLE_DIR / "main_final_38b_simulation_progress_by_100.csv"
...
def build_progress_checkpoints(runs: pd.DataFrame, actual: pd.DataFrame, step: int = 100) -> pd.DataFrame:
    """
    100회, 200회, ..., N회까지 누적 placebo 표본을 늘려가며
    OOS 핵심 지표의 유리 백분위가 어떻게 안정화되는지 계산한다.

    목적은 단순히 최종 1000회 결과만 보여주는 것이 아니라,
    Monte Carlo 검정이 100회 단위로 누적 수행되었고, 표본 수가 늘어날수록
    핵심 백분위가 어떤 경로로 수렴했는지 보고서에서 시각적으로 보여주는 데 있다.
    """
    if runs.empty:
        return pd.DataFrame()

    max_sim = int(runs["sim_id"].max()) + 1
    checkpoints = list(range(step, max_sim + 1, step))
    if checkpoints[-1] != max_sim:
        checkpoints.append(max_sim)

    rows: List[Dict[str, float | str | int]] = []
    for k in checkpoints:
        subset = runs.loc[runs["sim_id"] < k].copy()
        sub_summary = summarize_placebo(subset, actual)
        oos = sub_summary.loc[
            sub_summary["period"].eq("OOS") & sub_summary["metric"].isin(CORE_REPORT_METRICS)
        ].copy()
        for _, r in oos.iterrows():
            rows.append({
                "checkpoint": k,
                "progress_pct": k / max_sim * 100.0,
                "period": "OOS",
                "metric": r["metric"],
                ...
                "actual": r["actual"],
                "null_p50": r["null_p50"],
                "actual_advantage_percentile": r["actual_advantage_percentile"],
                "one_sided_p_value": r["one_sided_p_value"],
            })
    return pd.DataFrame(rows)


def save_progress_count_figure(progress: pd.DataFrame) -> Path:
    """100회 단위로 1000회까지 누적 실행했다는 점을 보여주는 진행률 그림을 저장한다."""
    base = progress[["checkpoint", "progress_pct"]].drop_duplicates().sort_values("checkpoint")
    fig, ax = plt.subplots(figsize=(8.8, 4.8))
    ax.plot(base["checkpoint"], base["progress_pct"], marker="o")
    ax.set_ylim(0, 105)
    ax.set_xlim(0, max(N_SIMULATIONS, int(base["checkpoint"].max())) * 1.03)
    ax.set_xticks(base["checkpoint"])
    ax.set_xlabel("Number of placebo simulations")
    ax.set_ylabel("Progress (%)")
    ax.set_title("38b. Monte Carlo progress by 100-simulation checkpoints")
    ax.grid(True, axis="y", alpha=0.3)
    for x, y in zip(base["checkpoint"], base["progress_pct"]):
        ax.text(x, y + 2, f"{int(x)}", ha="center", va="bottom", fontsize=8)
    fig.tight_layout()
    out = FIGURE_DIR / "main_final_fig_38b_monte_carlo_progress_by_100.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return out


def save_percentile_stability_figure(progress: pd.DataFrame) -> Path:
    """누적 시뮬레이션 수가 늘어날 때 OOS 핵심 백분위가 어떻게 변하는지 저장한다."""
    plot_metrics = ["net10_cagr_pct", "net10_mdd_pct", "net10_calmar", "net10_sharpe"]
    labels = {
        "net10_cagr_pct": "CAGR",
        "net10_mdd_pct": "MDD",
        "net10_calmar": "Calmar",
    ...
    for metric in plot_metrics:
        tmp = progress.loc[progress["metric"].eq(metric)].sort_values("checkpoint")
        if tmp.empty:
            continue
        ax.plot(tmp["checkpoint"], tmp["actual_advantage_percentile"], marker="o", label=labels.get(metric, metric))
    ax.axhline(95, linestyle="--", linewidth=1, label="95%ile")
    ax.axhline(80, linestyle=":", linewidth=1, label="80%ile")
    ax.axhline(50, linestyle="-.", linewidth=1, label="50%ile")
    ax.set_ylim(0, 100)
    ax.set_xticks(sorted(progress["checkpoint"].drop_duplicates()))
    ax.set_xlabel("Number of placebo simulations")
    ax.set_ylabel("Advantage percentile")
    ax.set_title("38b. OOS percentile stability by 100-simulation checkpoints")
    ax.grid(True, axis="y", alpha=0.3)
    ax.legend(ncol=2)
    fig.tight_layout()
    out = FIGURE_DIR / "main_final_fig_38b_oos_percentile_stability_by_100.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return out
...
def build_report_section(
    ...
    lines.append("")
    lines.append(f"![그림 1. Monte Carlo 100회 단위 진행 확인]({relpath_for_docs(progress_fig)})")
    lines.append("")
    lines.append("[그림 1. Monte Carlo 100회 단위 진행 확인]")
    lines.append(
        f"그림 1은 placebo simulation이 100회 단위로 누적되어 최종 {N_SIMULATIONS}회까지 수행되었음을 보여준다. "
        "이 그림은 최종 결과가 단일 셔플에 의존한 것이 아니라, 반복 표본을 단계적으로 누적해 만든 Monte Carlo 검정 결과임을 시각적으로 확인하기 위한 자료이다."
    )
    lines.append("")
    lines.append(f"![그림 2. OOS 핵심 백분위의 누적 안정화 경로]({relpath_for_docs(stability_fig)})")
    lines.append("")
    lines.append("[그림 2. OOS 핵심 백분위의 누적 안정화 경로]")
    lines.append(
        "그림 2는 100회, 200회, ..., 1000회로 placebo 표본을 늘려가며 OOS 핵심 지표의 유리 백분위가 어떻게 변하는지 보여준다. "
        "표본 수가 증가해도 주요 지표의 백분위가 크게 흔들리지 않으면, 최종 1000회 결과가 특정 소수 셔플의 우연에만 의존하지 않았다고 해석할 수 있다."
    )
    lines.append("")

    fig_specs = [
        ("net10_cagr_pct", "그림 3. OOS Net10bp CAGR placebo 분포와 실제 dynamic_v1 위치"),
        ("net10_mdd_pct", "그림 4. OOS Net10bp MDD placebo 분포와 실제 dynamic_v1 위치"),
        ("net10_calmar", "그림 5. OOS Net10bp Calmar placebo 분포와 실제 dynamic_v1 위치"),
        ("net10_tail_strategy_avg_pct", "그림 6. OOS Net10bp tail-month 평균수익 placebo 분포와 실제 dynamic_v1 위치"),
    ]
    ...
    lines.append(f"![그림 7. OOS 핵심 지표별 유리 백분위 요약]({relpath_for_docs(percentile_fig)})")
    lines.append("")
    lines.append("[그림 7. OOS 핵심 지표별 유리 백분위 요약]")
    lines.append(
        "그림 7은 OOS 구간의 핵심 지표별 유리 백분위를 요약한 것이다. 수익률 지표와 방어 지표는 값이 클수록 유리하게, "
...
def main() -> None:
    ...
    print("[6] 그림 생성")
    progress = build_progress_checkpoints(runs, actual_metrics, step=100)
    progress_fig = save_progress_count_figure(progress)
    stability_fig = save_percentile_stability_figure(progress)
    figure_paths = save_distribution_figures(actual_metrics, runs)
    percentile_fig = save_percentile_bar(summary)
    print(f"    진행 확인 그림 저장: {progress_fig}")
    print(f"    백분위 안정화 그림 저장: {stability_fig}")
    ...
    progress.to_csv(OUTPUT_PROGRESS_CHECKPOINTS, index=False, encoding="utf-8-sig")
    print(f"    저장: {OUTPUT_PROGRESS_CHECKPOINTS}")

    report_section = build_report_section(summary, report_comp, figure_paths, percentile_fig, progress_fig, stability_fig, max_diff)
