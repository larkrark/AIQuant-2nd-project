# -*- coding: utf-8 -*-
"""
16_regime_robustness_check.py

목적
----
최종 후보로 남은 Lambda 0.1 / Lambda 0.3이 전체 기간에서만 좋아 보인 것인지,
아니면 기간별·HSI 상태별·큰 손실월에서도 버틸 수 있는지 점검한다.

핵심 원칙
---------
1. 새 후보를 만드는 실험이 아니라, 남은 후보를 흔들어 보는 검증 실험이다.
2. 비교 대상은 소수로 제한한다.
   - EW Benchmark
   - HSI baseline
   - Lambda 0.1
...
output/figures/main_final_regime_robustness_period_cagr.png
output/figures/main_final_regime_robustness_period_mdd.png
output/figures/main_final_regime_robustness_state_avg_return_heatmap.png

docs/main_final_regime_robustness_note.md
"""
...
OUTPUT_STRATEGY_TS = DATA_PROCESSED / "main_final_regime_robustness_strategy_timeseries.csv"
OUTPUT_SUMMARY = OUTPUT_TABLES / "main_final_regime_robustness_summary.csv"
OUTPUT_BY_PERIOD = OUTPUT_TABLES / "main_final_regime_robustness_by_period.csv"
OUTPUT_BY_STATE = OUTPUT_TABLES / "main_final_regime_robustness_by_hsi_state.csv"
OUTPUT_TAIL_SUMMARY = OUTPUT_TABLES / "main_final_regime_robustness_tail_event_summary.csv"
OUTPUT_TAIL_MONTHS = OUTPUT_TABLES / "main_final_regime_robustness_tail_months.csv"
OUTPUT_DECISION = OUTPUT_TABLES / "main_final_regime_robustness_decision_note.csv"

OUTPUT_FIG_PERIOD_CAGR = OUTPUT_FIGURES / "main_final_regime_robustness_period_cagr.png"
OUTPUT_FIG_PERIOD_MDD = OUTPUT_FIGURES / "main_final_regime_robustness_period_mdd.png"
OUTPUT_FIG_STATE_HEATMAP = OUTPUT_FIGURES / "main_final_regime_robustness_state_avg_return_heatmap.png"
OUTPUT_NOTE = DOCS_DIR / "main_final_regime_robustness_note.md"
...
def build_overall_summary(ts: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["role"] = np.where(
        ...
        "candidate",
        np.where(summary["strategy_name"].eq("EW Benchmark"), "benchmark", "baseline"),
    )
...
def build_by_period(ts: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values(["period", "strategy_name"]).reset_index(drop=True)
            ...
            [
                {
                    ...
                    "finding": f"전체 기간 Calmar 기준 우위 후보는 {best_calmar['strategy_name']}입니다.",
...
def plot_period_metric(by_period: pd.DataFrame, metric: str, output_path: Path, title: str, ylabel: str) -> None:
    ...
    pivot = pivot[[c for c in ["EW Benchmark", "HSI baseline", "Lambda 0.1", "Lambda 0.3"] if c in pivot.columns]]
...
def plot_state_heatmap(by_state: pd.DataFrame) -> None:
    ...
    cols = [c for c in ["EW Benchmark", "HSI baseline", "Lambda 0.1", "Lambda 0.3"] if c in pivot.columns]
    pivot = pivot[cols]
...
def write_note(summary: pd.DataFrame, by_period: pd.DataFrame, by_state: pd.DataFrame, tail_summary: pd.DataFrame, decision: pd.DataFrame) -> None:
    ...
    text = f"""# 16번 Regime Robustness Check Note
...
def main() -> None:
    ...
    print("=" * 80)
    print("16_regime_robustness_check.py 실행 시작")
    print("=" * 80)

    print("[1] 전략별 월별 수익률 생성")
    ts = build_strategy_timeseries()
    print(f"    strategy timeseries shape = {ts.shape}")
    print(f"    strategies = {sorted(ts['strategy_name'].unique().tolist())}")
    ...
    plot_period_metric(
        ...
        title="Regime robustness by period: CAGR",
        ylabel="CAGR (%)",
    )
    plot_period_metric(
        by_period,
        ...
        title="Regime robustness by period: MDD",
        ylabel="MDD (%)",
    )
    ...
    print("=" * 80)
    print("16_regime_robustness_check.py 실행 완료")
    print("=" * 80)
