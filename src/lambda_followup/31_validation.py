...
def main() -> None:
    ...
    # ------------------------------------------------------------
    # dynamic_v1_macro — E30-M Macro-aware dynamic λ
    # ------------------------------------------------------------
    ...
    bt_dynamic_macro = X.run_lambda_backtest(
        returns,
        target_w,
        lambda_up=np.nan,
        lambda_down=np.nan,
    ...
    backtests["dynamic_v1_macro"] = bt_dynamic_macro
    ...
    audit.append({"항목": "rolling 계산 미래값 금지",
                  "자동판정": "코드 규약(30번·30-M: rolling만 사용, 전구간 z 금지) — 수동 확인 병행"})
    audit.append({"항목": "grid 사전고정", "자동판정": "config.py 고정 — 변경 이력 주석 확인"})
    audit.append({"항목": "MacroRisk 조건 반영",
                "자동판정": "30-M에서 factor_inputs_monthly.csv의 MacroRisk 열을 읽어 macro_risk_score>=2 조건으로 사용"})
