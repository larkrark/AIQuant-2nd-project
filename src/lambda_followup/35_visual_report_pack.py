...
def plot_weights(strategy: str) -> Path:
    ...
    # 보고서 삽입 시 제목·범례·OOS 라벨이 겹치지 않도록 세로를 약간 키움
    ...
    # 차트 제목은 짧게 유지하고, 상세 설명은 보고서 캡션에서 처리
    strategy_title = {
        "dynamic_v1": "dynamic_v1",
        "dynamic_v1_macro": "dynamic_v1_macro",
    }.get(strategy, strategy)

    ax.set_title(
        f"{strategy_title}\n리밸런싱 일자별 포트폴리오 구성 비중",
        fontsize=12,
        pad=18,
    )
...
def plot_adoption_decision() -> Path:
    ...
    metrics = [
        ("calmar_net", "OOS net 10bp Calmar"),
        ("mdd_pct", "OOS net 10bp MDD (%)"),
        ("tail_avg_pct", "Tail-month 평균수익 (%)"),
        ("avg_turnover_pct", "평균 Turnover (%)"),
    ]

    # 채택 기준선(34번 출력 결과 기준)
    calmar_cut = 0.965   # 0.9 × 대칭 최우수 Calmar_net(≈1.072)
    mdd_cut = -16.79     # 기준 MDD -14.79% 에서 2%p 악화 허용
    tail_cut = -5.03     # 기준 tail -4.73% 에서 0.3%p 악화 허용
    turnover_cut = 11.27 # 대칭 λ=0.3 평균 Turnover × 1.5

    cut_map = {
        "calmar_net": calmar_cut,
        "mdd_pct": mdd_cut,
        "tail_avg_pct": tail_cut,
        "avg_turnover_pct": turnover_cut,
    }

    for ax, (col, title) in zip(axes, metrics):
        bars = ax.barh(df["strategy_label"], df[col])
        ax.set_title(title)
        ax.grid(True, axis="x", alpha=0.25)
        ax.invert_yaxis()  # 위에서 아래로 읽기 쉽게

        # 기준선 표시
        if col in cut_map:
            ax.axvline(cut_map[col], linestyle="--", linewidth=1.2)
        ...
        for bar in bars:
            ...
            # 음수/양수에 따라 글자 위치 조정
            if value >= 0:
                x = value + (abs(value) * 0.02 + 0.02)
                ha = "left"
            else:
                x = value - (abs(value) * 0.02 + 0.02)
                ha = "right"

            ax.text(x, y, f"{value:.2f}", va="center", ha=ha, fontsize=9)
